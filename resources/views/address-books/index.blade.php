<x-app-layout>
    ...
    <x-slot name="meta_title">{{ __('address_book') }}</x-slot>
	<x-slot name="meta_desc">{{ __('address_book') }}</x-slot>
    <x-slot name="title">{{ __('address_book') }}</x-slot>

    <x-slot name="submit">
        <a href="{{ route('address-book.create') }}" class="p-2.5 pl-4 duration-300 shadow rounded-l-full bg-teal-600 text-white hover:bg-teal-700">
    ...
    <div class="p-2 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        ...
        @if ($addressBooks->isEmpty())
            <p class="py-4 italic text-center font-light lowercase dark:text-gray-400">{{ __('no_contacts') }}</p>
        @else
                ...
                <table class="table-auto w-full text-sm">
                    <thead>
                        <tr class="lowercase bg-gray-100 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400">
                            ...
                            <th class="px-2 pt-4 pb-2 text-left">{{ __('company') }}</th>
                            <th class="px-2 pt-4 pb-2 text-left">{{ __('first_and_last_name') }}</th>
                            <th class="px-2 pt-4 pb-2 text-left">{{ __('city') }}</th>
                            <th class="px-2 pt-4 pb-2 text-left">{{ __('email') }}</th>
                            <th class="px-2 pt-4 pb-2 text-center">{{ __('phone') }}</th>
