<x-app-layout>
	...
	<x-slot name="meta_title">{{ __('new_page') }}</x-slot>
	<x-slot name="meta_desc">{{ __('new_page') }}</x-slot>
	<x-slot name="title">{{ __('new_page') }}</x-slot>
	...
	<div x-data="{ group: 'tab-general', lang: 'tab-{{ app()->getLocale() }}' }">
		...
		<div class="relative">
			<form method="POST" action="{{ route('page.store') }}" id="form-store">
				@csrf
