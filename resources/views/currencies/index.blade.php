<x-app-layout>
    ...
    <x-slot name="meta_title">{{ __('currency') }}</x-slot>
            ...
            <table class="table-auto w-full text-sm">
                ...
                @foreach ($currencies as $currency)
                    <tr class="duration-300 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-900">
                        <td class="p-2 w-20 min-w-20 text-center">{{ $currency->id }}</td>
                        <td class="p-2 w-12 min-w-12 text-center">{{ $currency->code }}</td>
                        <td class="p-2 whitespace-nowrap">{{ $currency->name }}</td>
                        <td class="p-2 w-16 min-w-16 text-center">{{ $currency->priority }}</td>
                        <td class="p-2 w-16 min-w-16 text-center">
                            @if($default == $currency->id)
                        ...
                        <td class="p-2 w-12 min-w-12 text-center">
                            <a href="{{ route('currency.edit', $currency->id) }}" class="inline-block duration-300 text-gray-300 hover:text-gray-700 dark:text-gray-500 dark:hover:text-gray-300">
