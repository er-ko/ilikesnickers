<x-app-layout>
    ...
    <x-slot name="meta_title">{{ __('messages.customer_groups') }}</x-slot>
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('customer-group.create') }}" target="_self" class="px-2 text-teal-600 hover:text-teal-700">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="p-2 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                @if ($groups->isEmpty())
                    <p class="py-4 italic text-center font-light lowercase dark:text-gray-400">{{ __('messages.no_customer_groups') }}</p>
                @else
                            ...
                            @foreach ($groups as $group)
                                <tr class="border-b border-gray-100 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700 transition duration-400">
                                    ...
                                    <td class="p-2 w-24 min-w-24 text-center">
                                        <a href="{{ route('customer-group.edit', $group->id) }}" class="inline-block text-gray-300 hover:text-gray-700 dark:text-gray-500 dark:hover:text-gray-300 transition duration-400 relative top-0.5 mx-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                            </svg>
                                        </a>
                                        <form method="POST" action="{{ route('customer-group.destroy', $group->id) }}" class="inline-block relative top-0.5" x-data>
                                            @csrf
                                            @method('DELETE')
