<x-app-layout>
	...
	<x-slot name="meta_title">{{ __('new_product') }}</x-slot>
	<x-slot name="meta_desc">{{ __('new_product') }}</x-slot>
    <x-slot name="title">{{ __('new_produtct') }}</x-slot>
	...
	<x-slot name="submit">
		...
		<a href="{{ route('product.index') }}" class="py-2.5 px-3 duration-300 bg-pink-600 text-white hover:bg-pink-700 dark:bg-pink-700 dark:hover:bg-pink-600">
			<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
	...
	<div x-data="{ group: 'tab-general', lang: 'tab-{{ app()->getLocale() }}' }">
		...
		<div class="relative">
			<form method="POST" action="{{ route('product.store') }}" id="form-store" enctype="multipart/form-data">
				@csrf
				<div x-show="group == 'tab-general'">
					@include('products.partials.general')
				</div>
				<div x-show="group == 'tab-content'">
					@include('products.partials.content')
				</div>
				<div x-show="group == 'tab-image'">
					@include('products.partials.image')
				</div>
				<div x-show="group == 'tab-price'">
