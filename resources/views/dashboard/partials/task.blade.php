<x-card class="w-full max-w-md lg:p-8">
	<x-slot name="content">
		<div class="absolute top-0 sm:-top-2.5 right-0 sm:-right-2.5 w-10 h-10 flex items-center justify-center font-bold text-xs shadow-md sm:rounded-full {{ $color }} text-white {{ $tasks_total == 0 ? 'dark:text-black' : '' }}">
			{{ round($progress) }}%
		</div>
		<h3 class="mt-2 mb-4 text-2xl font-semibold uppercase flex items-center justify-start">
		...
		<div class="flex items-center justify-start space-x-2 mb-4">
			<div class="py-2 px-4 sm:rounded-lg bg-gray-800 text-white dark:bg-white dark:text-black">
				{{ __('total') }}: <span class="font-bold">{{ $tasks_total }}</span>
			</div>
			<div class="py-2 px-4 sm:rounded-lg bg-pink-700 text-white">
				{{ __('opened') }}: <span class="font-bold">{{ $tasks_opened }}</span>
			</div>
			<div class="py-2 px-4 sm:rounded-lg bg-teal-600 text-white">
				{{ __('closed') }}: <span class="font-bold">{{ $tasks_closed }}</span>
			</div>
		</div>
		<div class="absolute left-0 bottom-0 w-full sm:rounded-b-lg bg-gray-200">
