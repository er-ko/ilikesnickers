<x-app-layout>
	...
	<x-slot name="meta_title">{{ __('edit_country') }}</x-slot>
	<x-slot name="meta_desc">{{ __('edit_country') }}</x-slot>
	<x-slot name="title">{{ __('edit_country') }}</x-slot>
    ...
    <form method="POST" action="{{ route('country.update', $country) }}" id="form-store" class="h-full flex items-center justify-center">
        @csrf
        @method('patch')
        <div class="w-full max-w-md">
            @include('countries.partials.general')    
        </div>
    </form>
