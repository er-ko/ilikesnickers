@auth
<x-app-layout>
	<x-slot name="meta_title">{{ __('messages.faq') }}</x-slot>
	<x-slot name="meta_desc">{{ __('messages.faq') }}</x-slot>
    <x-slot name="title">{{ __('messages.faq') }}</x-slot>
    ...
    <div class="p-2 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        @if ($faqs->isEmpty())
            <p class="py-4 italic text-center font-light dark:text-gray-400">{{ __('messages.no_questions_and_answers') }}</p>
        @else
                ...
                <table class="table-auto w-full text-sm">
                    <thead>
                        <tr class="lowercase bg-gray-100 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400">
                            ...
                            <th class="px-2 pt-4 pb-2 text-left">{{ __('messages.question') }}</th>
