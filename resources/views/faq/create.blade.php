<x-app-layout>
	...
	<x-slot name="meta_title">{{ __('messages.new_faq') }}</x-slot>
	<x-slot name="meta_desc">{{ __('messages.new_faq') }}</x-slot>
	<x-slot name="title">{{ __('messages.new_faq') }}</x-slot>
	...
	<div x-data="{ group: 'tab-general', lang: 'tab-{{ app()->getLocale() }}' }">
		...
		<div class="relative">
			<form method="POST" action="{{ route('faq.store') }}" id="form-store">
				@csrf
