<div class="accordion w-full h-fit rounded-xl shadow dark:text-gray-300">
	@foreach($faqs as $faq)
		<div class="accordion-item bg-white border-b border-gray-100 dark:bg-slate-800 dark:border-slate-700/50">
			<div class="accordion-header flex items-center justify-between p-8 hover:cursor-pointer">
				<h3 class="text-lg font-semibold lowercase"># {{ $faq->question }}</h3>
...
@push('slotscript')
	<script>
		$(document).ready(function(){
			$('.accordion').find('.accordion-item').first().addClass('rounded-t-lg');
			$('.accordion').find('.accordion-item').last().addClass('rounded-b-lg');
