<x-card class="lg:p-8 space-y-4">
	<x-slot name="content">
		...
		<div>
			...
			<x-text-input id="company-name" name="contact_company_name" type="text" maxlength="255" :value="old('company_name', $contact['company_name'])" />
		...
		<div>
			...
			<textarea
				...
				rows="3"
				maxlenth="255"
			>{{ old('contact_address', $contact['address']) }}</textarea>
			<x-input-error :messages="$errors->get('address')" />
		</div>
		<div>
			<x-input-label for="phone" :required="false" :value="__('phone')" />
			<x-text-input id="phone" name="contact_phone" type="text" maxlength="16" :value="old('contact_phone', $contact['phone'])" />
			<x-input-error :messages="$errors->get('phone')" />
		</div>
		<div>
			<x-input-label for="email" :required="false" :value="__('email')" />
			<x-text-input id="email" name="contact_email" type="email" maxlength="255" placeholder="@" :value="old('contact_email', $contact['email'])" />
			<x-input-error :messages="$errors->get('email')" />
		</div>
		<div>
			<x-input-label for="web" :required="false" :value="__('web')" />
			<x-text-input id="web" name="contact_web" type="text" maxlength="255" :value="old('contact_web', $contact['web'])" />
			<x-input-error :messages="$errors->get('web')" />
		</div>
		<div>
			<x-input-label for="map" :required="false" :value="__('map')" />
			<x-text-input id="map" name="contact_map" type="text" maxlength="255" :value="old('contact_map', $contact['map'])" />
			<x-input-error :messages="$errors->get('map')" />
		</div>
		<div>
			<x-input-label for="whatsapp" :required="false" :value="__('whatsapp')" />
			<x-text-input id="whatsapp" name="contact_whatsapp" type="text" maxlength="16" :value="old('contact_whatsapp', $contact['whatsapp'])" />
